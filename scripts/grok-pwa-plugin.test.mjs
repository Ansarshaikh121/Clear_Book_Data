...
/** Tests that assert generic injector behavior must not see this app's card. */
...
test("page title, description, and site.host replace the grok.me share card", () => {
  const prev = process.env.VITE_PUBLIC_HOSTNAME;
  process.env.VITE_PUBLIC_HOSTNAME = "blade-sage-zinc-ember.grok.me";
  try {
    const html =
      ...
      "</head></html>";
    const out = injectGrokPwaHead(html, {
      host: "blade-sage-zinc-ember.grok.me",
      site: {
        title: "Clearbook",
        description: "fallback that must not win",
        card: "custom",
        host: "clearbookdata.in",
      },
    });
    assert.match(
      out,
      /property="og:title" content="Clearbook — a personal ledger for income, expenses, and savings"/,
    );
    assert.match(
      out,
      /name="twitter:title" content="Clearbook — a personal ledger for income, expenses, and savings"/,
    );
    assert.match(
      out,
    ...
    assert.match(out, /name="twitter:image" content="https:\/\/clearbookdata\.in\/og\.jpg"/);
    assert.match(out, /property="og:url" content="https:\/\/clearbookdata\.in\/"/);
    assert.match(out, /property="og:site_name" content="Clearbook"/);
    assert.doesNotMatch(out, /og\.grok\.me/);
    assert.doesNotMatch(out, /blade-sage-zinc-ember/);
    assert.doesNotMatch(out, /fallback that must not win/);
    const twice = injectGrokPwaHead(out, {
      host: "blade-sage-zinc-ember.grok.me",
      site: { title: "Clearbook", card: "custom", host: "clearbookdata.in" },
    });
    assert.equal(twice.split('property="og:image"').length - 1, 1);
    assert.equal(twice.split('name="twitter:image"').length - 1, 1);
    assert.equal(twice.split('property="og:description"').length - 1, 1);
  } finally {
    if (prev === undefined) delete process.env.VITE_PUBLIC_HOSTNAME;
    else process.env.VITE_PUBLIC_HOSTNAME = prev;
  }
...
test("injects into documents with no head element", () => {
  const out = injectGrokPwaHead("<html><body>hi</body></html>", { appName: "Solo", cwd: isolatedCwd() });
...
test("uses the app name in the injected title tag", () => {
  const out = injectGrokPwaHead("<html><head></head></html>", { appName: "Wild Race", cwd: isolatedCwd() });
