...
// Exact public origins. No wildcards and no http:// variants — a lookalike host
...
let configuredHttpsOrigin: string | null = null;
let configuredHttpsHost: string | null = null;
if (explicitBaseURL) {
  try {
    const configured = new URL(explicitBaseURL);
    if (configured.protocol === "https:") {
      configuredHttpsOrigin = configured.origin;
      configuredHttpsHost = configured.hostname;
    }
  } catch {
    configuredHttpsOrigin = null;
    configuredHttpsHost = null;
  }
}

const baseURL = explicitBaseURL
  ? {
      // Request host is used only when it is one of these names, so OAuth
      // callbacks and reset links stay on the site the person is using.
      // Anything else falls back to BETTER_AUTH_URL instead of being trusted.
      allowedHosts: configuredHttpsHost
        ? [...new Set([...publicAppHosts, configuredHttpsHost])]
        : [...publicAppHosts],
...
const trustedOrigins: string[] = [
  ...
  ...(explicitBaseURL
    ? []
    : [
        // Host wildcards (matched against Origin's host) — preview only.
        ...previewAllowedHosts,
        // Full-origin wildcards (matched against Origin) — preview only.
